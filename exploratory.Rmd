...
indicators <- c("SP.URB.TOTL.IN.ZS", "NY.GDP.MKTP.CD", "NE.CON.GOVT.CD",
...
b <- left_join(a, tax_comb, by = c("country", "year"))
c <- plm.data(b)
...
test <- a
c2 <- select(c1, iso2c = iso2, join)

asd <- left_join(wdi_controls, c2 , by = "iso2c") %>%
  mutate(OECD = ifelse(year >= join, TRUE, FALSE)) %>%
  select(-join)
